...
C-----VARIABLE DECLARATIONS---------------------------------------------
      ...
      double precision moc, inodens
c      double precision bcden, ocden 
...
C-----ADJUSTABLE PARAMETERS---------------------------------------------
     ...
     &          mws2=2*mwnh4+mwso4)
c      parameter(bcden=2200., ocden=1400. )
C-----CODE--------------------------------------------------------------
...
C Calculate mole fractions
      ...
      d=1.5d0 !dlm, 02/2019 ; copied from TOMAS_MOD in GEOS-Chem
      go to 10
      endif
      nso4 = so4t/mwso4
      nno3 = no3t/mwno3
...
Cdbg      print*, 'AERODENS sulfate xan xnacl & dan dss', xan,xnacl,dan,dss
...
Cdbg      print*, 'AERODENS total fraction solute',xtot
...
Cdbg      print*,'reciprocal vals ', xan,dan,xs0,ds0,xs1,ds1,
Cdbg     &                                xs2,ds2,xnacl,dss

      MTOT = MSO4+MNO3+MNH4+MNACL+MH2O+MOC
      IF ( MTOT > 0.d0 ) THEN
         AERODENS = ( INODENS*(MSO4+MNO3+MNH4+MNACL+MH2O) +
     &              OCDEN*MOC )/MTOT
      ELSE
         AERODENS = 1400.
      ENDIF
c      print*,'DENSITY:', aerodens
      RETURN
      END
