...
C     The program updates Nk and Mk.
      ...
      SUBROUTINE multicoag(dt, coagsink)
...
cdbg      character*12 limit        !description of what limits time step
      double precision coagsink(ibins-1)
c      logical          exist
c      character*80     fullname, fname

      double precision mi, mf   !initial and final masses

      double precision tot_m, tot_n
      integer Ndts !number of adaptive time steps used
C     VARIABLE COMMENTS...
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
         density=aerodens(Mk(k,srtso4),0.d0,
     &           0.1875d0*Mk(k,srtso4),Mk(k,srtna),  !assume bisulfate
     &           Mk(k,srth2o),Mk(k,srtc))

         Mktot=0.2*Mk(k,srtso4) ! account for ammonia
         do j=1,icomp
            Mktot=Mktot+Mk(k,j)
         enddo
         mp = Mktot/Nk(k)
...
Cdbg      print*, '#multicoag Nk phi eff xbar', Nk, phi, eff, xbar
...
c      print*, '#Multicoag dmdt ', (dMdt(k,srtso4),k=1,ibins)
...
Cdbg      print*, '#Multicoag Mk SO4', (Mk(k,srtso4),k=1,ibins)
      tot_n = 0.d0
      tot_m = 0.d0
      do k=1,ibins
         ...
         enddo
      enddo
Cdbg      print*,' #Multicoag total dry mass = ',tot_m, 'total Numb= ',
Cdbg     &           tot_n
Cdbg      print*, '#Multicoag dmdt SO4', (dMdt(k,srtso4),k=1,ibins)
      ...
      do i=1,ibins-1
         coagsink(i) = 1/2*kij(i,i)*Nk(i)
         do k=i+1,ibins
           coagsink(i) = coagsink(i) + kij(i,k)*Nk(k)
         enddo
      enddo

c      print*,'#MULTICOAG number of timesteps= ', Ndts
      RETURN
      END
